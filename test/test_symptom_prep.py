...
class TestSymptomPrep(object):
    def test_copy_variables(self, prep):
        ...
        row = dict(list(zip(headers, [2, 37, 0, 0])))
        ...
        assert row == dict(list(zip(headers, [2, 37, 2, 37])))

    def test_process_progressive_data(self, prep):
        headers = ['test0', 'test1', 'test2', 'test3', 'test4']
        rows = [
            dict(list(zip(headers, [72, 0, 0, 0, 0]))),
            dict(list(zip(headers, [12, 0, 0, 0, 0]))),
            dict(list(zip(headers, [35, 0, 0, 0, 0]))),
            dict(list(zip(headers, [48, 0, 0, 0, 0]))),
        ]

        valid_results = [
            dict(list(zip(headers, [72, 0, 0, 0, 1]))),
            dict(list(zip(headers, [12, 1, 0, 0, 0]))),
            dict(list(zip(headers, [35, 0, 1, 0, 0]))),
            dict(list(zip(headers, [48, 0, 0, 1, 0]))),
        ]
        ...
        for i, row in enumerate(rows):
            prep.process_progressive_value_data(row, list(conversion_map.items()))

            assert row == valid_results[i]

    def test_process_cutoff_data(self, prep):
        headers = ['test1', 'test2', 'test3', 'test4']
        row = dict(list(zip(headers, [0.12345, 40, 20, ''])))
        ...
        prep.process_cutoff_data(row, list(conversion_map.items()))

        assert row == dict(list(zip(headers, [1, 0, 1, 0])))

    def test_process_injury_data(self, prep):
        headers = ['test0', 'test1', 'test2', 'test3', 'test4']
        rows = [
            dict(list(zip(headers, [999, 1, 0, 1, 0]))),
            dict(list(zip(headers, [40, 1, 0, 1, 0]))),
            dict(list(zip(headers, [28, 1, 0, 1, 0]))),
        ]

        valid_results = [
            dict(list(zip(headers, [999, 0, 0, 0, 0]))),
            dict(list(zip(headers, [40, 0, 0, 0, 0]))),
            dict(list(zip(headers, [28, 1, 0, 1, 0]))),
        ]
        ...
        for i, row in enumerate(rows):
            prep.process_injury_data(row, list(conversion_map.items()))

            assert row == valid_results[i]

    def test_process_injury_data_missing(self, prep):
        headers = ['test1', 'test2', 'test3', 'test4']
        row = dict(list(zip(headers, [1, 0, 1, 0])))

        valid_result = dict(list(zip(headers, [1, 0, 1, 0])))
