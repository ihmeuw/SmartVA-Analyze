...
class Tariff(object):
    ...
    def run(self):
        ...
        status_logger.debug('Child :: Calculating scores for validated dataset.')
        ...
        for i, row in enumerate(validatedmatrix):
            ...
            for causenum in range(1, max_cause + 1):
                ...
                if cnt % max((total / div), 1) == 0:
                    status_logger.debug('Child :: Processing %s of %s' % (cnt, total))
        ...
        status_logger.debug('Child :: Processing %s of %s' % (total, total))
        status_notifier.update({'sub_progress': None})

        status_logger.debug('Child :: Creating uniform training set')
        ...
        status_logger.debug('Child :: Generating cause rankings.')
        ...
        status_logger.debug('Child :: Generating cutoffs')
